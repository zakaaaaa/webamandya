...
export default function VouchersClient({ vouchers, totalCount, totalPages, currentPage, clientId, stats, filters }: Props) {
  ...
  const discountColor = (v:Voucher) => v.discount_type==='full'
    ...
    ? { bg:'rgba(212,43,34,.12)',  color:'#E83530', border:'rgba(212,43,34,.2)' }
    : { bg:'rgba(245,158,11,.12)', color:'#D97706', border:'rgba(245,158,11,.2)' }
  ...
  return (
    <>
      <style>{`
        ...
        .btn{display:flex;align-items:center;gap:6px;padding:8px 16px;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;transition:all .2s;border:1px solid transparent;font-family:'Poppins',sans-serif}
        ...
        .inp{width:100%;background:rgba(212,43,34,0.06);border:1px solid rgba(212,43,34,0.08);border-radius:10px;color:#150C09;font-size:13px;padding:9px 13px;outline:none;transition:all .2s;font-family:'Poppins',sans-serif}
        .inp::placeholder{color:rgba(158,136,128,0.95)} .inp:focus{border-color:rgba(212,43,34,.5);box-shadow:0 0 0 3px rgba(212,43,34,.12)}
        .inp-label{color:rgba(122,98,89,0.95);font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;margin-bottom:6px;display:block;font-family:'Poppins',sans-serif}
        .seg{display:flex;border-radius:10px;overflow:hidden;border:1px solid rgba(212,43,34,0.08);background:rgba(212,43,34,0.04)}
        .seg-btn{flex:1;padding:8px 4px;font-size:12px;font-weight:600;cursor:pointer;border:none;background:transparent;color:rgba(122,98,89,0.88);transition:all .15s;font-family:'Poppins',sans-serif}
        .seg-btn.active{background:linear-gradient(135deg,rgba(212,43,34,.3),rgba(212,43,34,.2));color:#E83530}
        .page-btn{width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;border:1px solid rgba(212,43,34,0.07);background:rgba(212,43,34,0.04);color:rgba(122,98,89,0.95);cursor:pointer;transition:all .15s;font-size:13px;font-weight:600}
        .page-btn:hover:not(:disabled){background:rgba(212,43,34,.15);border-color:rgba(212,43,34,.3);color:#E83530} .page-btn.active{background:linear-gradient(135deg,#E83530,#C02018);border-color:transparent;color:white} .page-btn:disabled{opacity:.3;cursor:default}
        .sinp{background:rgba(212,43,34,0.055);border:1px solid rgba(212,43,34,0.07);border-radius:10px;color:#150C09;font-size:13px;padding:8px 12px 8px 36px;outline:none;transition:all .2s;font-family:'Poppins',sans-serif;width:100%}
      ...
      <div style={{fontFamily:"'Poppins',sans-serif",minHeight:'100vh'}}>
        ...
        <div className="ph" style={{marginBottom:28}}>
          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:6}}>
            ...
            <p style={{color:'rgba(122,98,89,0.8)',fontSize:11,fontWeight:600,letterSpacing:'2.5px',textTransform:'uppercase',fontFamily:'Poppins,sans-serif'}}>Admin · Voucher</p>
          </div>
          <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
            ...
            <div style={{display:'flex',gap:8}}>
              ...
              <button className="btn" onClick={()=>{setForm(f=>({...f,code:generateCode()}));setModal('single');setError('')}} style={{background:'linear-gradient(135deg,#E83530,#C02018)',color:'#fff',boxShadow:'0 4px 16px rgba(212,43,34,.3)'}}>
                <Plus size={14}/>Buat Voucher
              </button>
            </div>
          </div>
          {success&&<div style={{marginTop:12,padding:'10px 16px',borderRadius:10,background:'rgba(16,185,129,.1)',border:'1px solid rgba(16,185,129,.2)',color:'#059669',fontSize:13,display:'flex',alignItems:'center',gap:8}}>
            <Check size={14}/>{success}<button onClick={()=>setSuccess('')} style={{marginLeft:'auto',background:'none',border:'none',color:'#059669',cursor:'pointer'}}><X size={12}/></button>
          </div>}
        </div>
        ...
        <div className="sh" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:24}}>
          {[
            ...
            {label:'Dipakai Hari Ini',value:stats.usedToday,color:'rgba(212,43,34,.25)',icon:Sparkles},
          ].map(({label,value,color,icon:Icon})=>(
            <div key={label} className="glass-card" style={{padding:20,boxShadow:`0 0 24px ${color}`}}>
              <p style={{color:'rgba(122,98,89,0.8)',fontSize:10,fontWeight:600,letterSpacing:'1px',textTransform:'uppercase',fontFamily:'Poppins,sans-serif',marginBottom:12}}>{label}</p>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <p style={{color:'#150C09',fontSize:26,fontWeight:700,fontFamily:'Poppins,sans-serif'}}>{value}</p>
                <div style={{width:34,height:34,borderRadius:10,background:color,display:'flex',alignItems:'center',justifyContent:'center'}}><Icon size={16} color="#fff"/></div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="tb glass-card" style={{padding:'12px 16px',marginBottom:18,display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
          <div style={{position:'relative',flex:1,minWidth:180}}>
            <Search size={13} style={{position:'absolute',left:11,top:'50%',transform:'translateY(-50%)',color:'rgba(122,98,89,0.8)',pointerEvents:'none'}}/>
          ...
          {(filters.status||filters.search)&&<button className="btn" onClick={()=>{setLocalSearch('');pushFilter({status:'',search:''})}} style={{background:'rgba(239,68,68,.08)',color:'#B82018',border:'1px solid rgba(239,68,68,.15)'}}><X size={13}/>Reset</button>}
          {isPending&&<div style={{width:15,height:15,border:'2px solid rgba(212,43,34,.3)',borderTopColor:'#D42B22',borderRadius:'50%',animation:'spin .8s linear infinite'}}/>}
          <span style={{marginLeft:'auto',color:'rgba(158,136,128,0.95)',fontSize:12}}>{totalCount} voucher</span>
        </div>
        ...
        <div className="tbl glass-card" style={{overflow:'hidden'}}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              ...
              <tbody>
                ...
                {vouchers.map(v=>{
                  ...
                  return (
                    <tr key={v.id} className="vrow" style={{borderBottom:'1px solid rgba(212,43,34,0.03)',transition:'background .15s'}}>
                      <td style={{padding:'14px 18px'}}>
                        <div style={{display:'flex',alignItems:'center',gap:8}}>
                          <code style={{color:bad?'rgba(122,98,89,0.8)':'#150C09',fontSize:15,fontWeight:700,letterSpacing:2,fontFamily:'monospace',textDecoration:bad?'line-through':'none'}}>{v.code}</code>
                          <button onClick={()=>copyCode(v.code,v.id)} style={{background:'none',border:'none',cursor:'pointer',color:'rgba(122,98,89,0.8)',padding:3,borderRadius:5,display:'flex'}}>
                      ...
                      <td style={{padding:'14px 18px'}}>
                        <div style={{minWidth:100}}>
                          <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                            <span style={{color:'rgba(74,46,34,0.9)',fontSize:12}}>{v.used_count} / {v.max_uses??'∞'}</span>
                            {full&&<span style={{color:'#B82018',fontSize:10,fontWeight:700}}>HABIS</span>}
                          </div>
                          {v.max_uses&&<div style={{height:3,borderRadius:2,background:'rgba(212,43,34,0.055)',overflow:'hidden'}}>
                            <div style={{height:'100%',width:`${usagePct(v)}%`,borderRadius:2,background:usagePct(v)>=100?'#B82018':usagePct(v)>=70?'#D97706':'#D42B22',transition:'width .3s'}}/>
                          </div>}
                        </div>
                      </td>
                      <td style={{padding:'14px 18px'}}>
                        <div style={{display:'flex',alignItems:'center',gap:5,color:expired?'#B82018':'rgba(74,46,34,0.82)',fontSize:12}}>
                          <Clock size={11}/>{formatDate(v.valid_until)}
                          {expired&&<span style={{fontSize:9,fontWeight:700,color:'#B82018'}}>EXPIRED</span>}
                        </div>
                      </td>
                      <td style={{padding:'14px 18px'}}>
                        <button onClick={()=>handleToggle(v)} disabled={togglingId===v.id} style={{display:'flex',alignItems:'center',gap:5,background:'none',border:'none',cursor:'pointer',transition:'all .2s'}}>
                          {togglingId===v.id?<div style={{width:14,height:14,border:'2px solid rgba(158,136,128,0.85)',borderTopColor:'#150C09',borderRadius:'50%',animation:'spin .8s linear infinite'}}/>
                            :v.is_active?<ToggleRight size={22} color="#059669"/>:<ToggleLeft size={22} color="rgba(158,136,128,0.85)"/>}
                          <span style={{fontSize:11,fontWeight:600,color:v.is_active?'#059669':'rgba(158,136,128,0.85)'}}>{v.is_active?'Aktif':'Nonaktif'}</span>
                        </button>
                      </td>
                      <td style={{padding:'14px 18px',color:'rgba(158,136,128,0.95)',fontSize:11,whiteSpace:'nowrap'}}>
                        {new Date(v.created_at).toLocaleDateString('id-ID',{day:'2-digit',month:'short',year:'numeric'})}
                      </td>
                      <td style={{padding:'14px 18px'}}>
                        <div style={{display:'flex',gap:6}}>
                          <button onClick={()=>copyCode(v.code,v.id+'_a')} title="Copy" style={{width:30,height:30,borderRadius:7,background:'rgba(212,43,34,.1)',border:'1px solid rgba(212,43,34,.15)',color:'#E83530',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
                            {copiedId===v.id+'_a'?<Check size={12} color="#059669"/>:<Copy size={12}/>}
                          </button>
                          <button onClick={()=>handleDelete(v.id)} disabled={deletingId===v.id} title="Hapus" style={{width:30,height:30,borderRadius:7,background:'rgba(239,68,68,.08)',border:'1px solid rgba(239,68,68,.12)',color:'#B82018',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
                            {deletingId===v.id?<div style={{width:11,height:11,border:'2px solid rgba(239,68,68,.3)',borderTopColor:'#B82018',borderRadius:'50%',animation:'spin .8s linear infinite'}}/>:<Trash2 size={12}/>}
                          </button>
                        </div>
                      </td>
                    </tr>
          ...
          {totalPages>1&&<div style={{padding:'16px 18px',borderTop:'1px solid rgba(212,43,34,0.05)',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
            ...
            {Array.from({length:totalPages},(_,i)=>i+1).filter(p=>p===1||p===totalPages||Math.abs(p-currentPage)<=2)
              ...
              .map((p,i)=>p==='...'?<span key={`d${i}`} style={{color:'rgba(158,136,128,0.85)',fontSize:12,padding:'0 3px'}}>…</span>
          ...
          <div style={{padding:22,display:'flex',flexDirection:'column',gap:18}}>
            ...
            <div>
              ...
              <div style={{display:'flex',gap:8}}>
                ...
                <button className="btn" onClick={()=>setForm(f=>({...f,code:generateCode()}))} style={{background:'rgba(212,43,34,0.06)',color:'rgba(74,46,34,0.9)',border:'1px solid rgba(212,43,34,0.08)',flexShrink:0,padding:'8px 12px'}}><RefreshCw size={13}/></button>
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                <button onClick={()=>setForm(f=>({...f,unlimited_uses:!f.unlimited_uses}))} style={{display:'flex',alignItems:'center',gap:6,background:'none',border:'none',cursor:'pointer',color:form.unlimited_uses?'#D42B22':'rgba(122,98,89,0.95)',fontSize:13,fontWeight:500}}>
                  {form.unlimited_uses?<ToggleRight size={20} color="#D42B22"/>:<ToggleLeft size={20}/>}Tidak terbatas
                </button>
              </div>
            ...
            <div>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                <button onClick={()=>setForm(f=>({...f,no_expiry:!f.no_expiry}))} style={{display:'flex',alignItems:'center',gap:6,background:'none',border:'none',cursor:'pointer',color:form.no_expiry?'#D42B22':'rgba(122,98,89,0.95)',fontSize:13,fontWeight:500}}>
        ...
        <div className="modal-box" onClick={e=>e.stopPropagation()} style={{background:'rgba(255,255,255,0.97)',border:'1px solid rgba(212,43,34,0.07)',borderRadius:20,width:'100%',maxWidth:480,overflow:'hidden'}}>
          ...
          <div style={{padding:22,display:'flex',flexDirection:'column',gap:18}}>
            ...
            <div>
              <label className="inp-label">Batas Berlaku</label>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                <button onClick={()=>setBulk(b=>({...b,no_expiry:!b.no_expiry}))} style={{display:'flex',alignItems:'center',gap:6,background:'none',border:'none',cursor:'pointer',color:bulk.no_expiry?'#D42B22':'rgba(122,98,89,0.95)',fontSize:13,fontWeight:500}}>
            ...
            <div style={{padding:'12px 14px',borderRadius:10,background:'rgba(212,43,34,.06)',border:'1px solid rgba(212,43,34,.12)'}}>
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {generateBulkCodes(3,bulk.prefix).map(c=>(
                  <code key={c} style={{color:'#E83530',fontSize:12,fontWeight:700,letterSpacing:1.5,fontFamily:'monospace',background:'rgba(212,43,34,.1)',padding:'3px 8px',borderRadius:5}}>{c}</code>
                ))}
                <span style={{color:'rgba(158,136,128,0.85)',fontSize:12,alignSelf:'center'}}>+{Math.max(0,bulk.count-3)} lainnya</span>
              </div>
            </div>
          </div>
          <div style={{padding:'16px 22px',borderTop:'1px solid rgba(212,43,34,0.055)',display:'flex',gap:10,justifyContent:'flex-end'}}>
      ...
      {modal==='print'&&printCodes.length>0&&<div className="modal-bg" onClick={()=>setModal(null)} style={{position:'fixed',inset:0,zIndex:200,background:'rgba(0,0,0,.88)',backdropFilter:'blur(12px)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
        <div className="modal-box" onClick={e=>e.stopPropagation()} style={{background:'rgba(255,255,255,0.97)',border:'1px solid rgba(212,43,34,0.07)',borderRadius:20,width:'100%',maxWidth:700,maxHeight:'85vh',overflow:'hidden',display:'flex',flexDirection:'column'}}>
          <div className="no-print" style={{padding:'18px 22px',borderBottom:'1px solid rgba(212,43,34,0.055)',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
            <div><h2 style={{color:'#150C09',fontSize:16,fontWeight:700,fontFamily:'Poppins,sans-serif',marginBottom:2}}>✅ {printCodes.length} Voucher Berhasil Dibuat</h2><p style={{color:'rgba(122,98,89,0.8)',fontSize:12}}>Simpan atau print untuk dibagikan ke customer</p></div>
            <div style={{display:'flex',gap:8}}>
              <button className="btn no-print" onClick={()=>window.print()} style={{background:'rgba(16,185,129,.1)',color:'#059669',border:'1px solid rgba(16,185,129,.2)'}}><Printer size={13}/>Print</button>
              <button onClick={()=>{setModal(null);setSuccess(`${printCodes.length} voucher berhasil dibuat!`)}} style={{width:30,height:30,borderRadius:8,background:'rgba(212,43,34,0.055)',border:'1px solid rgba(212,43,34,0.07)',color:'rgba(122,98,89,0.95)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button>
            </div>
          </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
              {printCodes.map((code,i)=>(
                <div key={i} style={{border:'2px dashed rgba(212,43,34,.3)',borderRadius:10,padding:'14px 10px',textAlign:'center',background:'rgba(212,43,34,.05)',position:'relative'}}>
                  ...
                  <p style={{color:'rgba(122,98,89,0.8)',fontSize:8,letterSpacing:'1.5px',marginBottom:4,fontFamily:'Poppins,sans-serif',textTransform:'uppercase'}}>Voucher Gratis</p>
                  <code style={{color:'#150C09',fontSize:13,fontWeight:800,letterSpacing:2,fontFamily:'monospace',display:'block',marginBottom:8}}>{code}</code>
                  <div style={{height:1,background:'rgba(212,43,34,0.055)',marginBottom:8}}/>
                  <p style={{color:'rgba(158,136,128,0.85)',fontSize:8,fontFamily:'monospace'}}>Photobooth · Gratis 1x</p>
                  <button className="no-print" onClick={()=>copyCode(code,`p${i}`)} style={{position:'absolute',top:6,right:6,background:'none',border:'none',cursor:'pointer',color:'rgba(158,136,128,0.85)',padding:3}}>
                    {copiedId===`p${i}`?<Check size={11} color="#059669"/>:<Copy size={11}/>}
                  </button>
                </div>
              ))}
            </div>
