...
export default function DownloadPage({
  ...
  const StripPreview = ({ scale=1, frameUrl }: { scale?:number; frameUrl:string|null }) => {
    ...
    return (
      <div style={{position:'relative',width:W,height:H,background:'white',flexShrink:0,overflow:'hidden'}}>
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        body{font-family:'Plus Jakarta Sans',sans-serif;background:#080614;overflow-x:hidden}
        ...
        .glass{background:rgba(255,255,255,.05);backdrop-filter:blur(24px) saturate(180%);border:1px solid rgba(255,255,255,.08)}
        ...
        .tab-btn{padding:8px 18px;border-radius:10px;border:1px solid rgba(255,255,255,.08);background:transparent;color:rgba(255,255,255,.35);font-size:13px;font-weight:600;cursor:pointer;transition:all .2s;font-family:'Plus Jakarta Sans',sans-serif;display:flex;align-items:center;gap:6px;white-space:nowrap}
        .tab-btn.active{background:rgba(99,102,241,.2);border-color:rgba(99,102,241,.5);color:#a5b4fc}
        .tab-btn:hover:not(.active){background:rgba(255,255,255,.05);color:rgba(255,255,255,.6)}
        .dl-btn{display:flex;align-items:center;justify-content:center;gap:8px;padding:12px 20px;border-radius:12px;border:none;font-size:14px;font-weight:600;cursor:pointer;transition:all .2s;font-family:'Plus Jakarta Sans',sans-serif;width:100%}
      ...
      <div style={{minHeight:'100vh',position:'relative',overflow:'hidden',background:'linear-gradient(135deg,#080614 0%,#150f2e 50%,#0d1525 100%)'}}>
        <div style={{position:'fixed',width:600,height:600,top:-150,left:-150,borderRadius:'50%',filter:'blur(80px)',pointerEvents:'none',zIndex:0,background:'radial-gradient(circle,rgba(99,102,241,.15) 0%,transparent 70%)',animation:'float-1 18s ease-in-out infinite'}}/>
        <div style={{position:'fixed',width:500,height:500,bottom:-100,right:-100,borderRadius:'50%',filter:'blur(80px)',pointerEvents:'none',zIndex:0,background:'radial-gradient(circle,rgba(139,92,246,.12) 0%,transparent 70%)',animation:'float-2 22s ease-in-out infinite'}}/>
        ...
        <div style={{position:'relative',zIndex:10,maxWidth:1000,margin:'0 auto',padding:'36px 20px 80px'}}>
          ...
          <div className="fu1" style={{textAlign:'center',marginBottom:32}}>
            <div style={{display:'inline-flex',alignItems:'center',gap:8,marginBottom:16,background:'rgba(99,102,241,.1)',border:'1px solid rgba(99,102,241,.25)',borderRadius:100,padding:'7px 16px'}}>
              <Camera size={14} color="#818cf8"/>
              <span style={{color:'#a5b4fc',fontSize:12,fontWeight:600}}>{clientName}</span>
            </div>
            <h1 style={{color:'white',fontSize:'clamp(24px,5vw,44px)',fontWeight:800,fontFamily:'Poppins,sans-serif',lineHeight:1.2,marginBottom:10}}>
              Foto kamu sudah siap! 🎉
            </h1>
            <p style={{color:'rgba(255,255,255,.4)',fontSize:14,maxWidth:480,margin:'0 auto'}}>
          ...
          <div className="fu2 glass" style={{borderRadius:14,padding:'12px 20px',marginBottom:24,display:'flex',alignItems:'center',gap:14,flexWrap:'wrap'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:6}}>
              <Clock size={13} color="rgba(255,255,255,.3)"/>
              <span style={{color:'rgba(255,255,255,.3)',fontSize:12}}>{formatDate(session.created_at)}</span>
            </div>
            <code style={{marginLeft:'auto',color:'rgba(255,255,255,.15)',fontSize:10,fontFamily:'monospace'}}>{uuid.slice(0,24)}...</code>
          </div>
          ...
          <div className="main-grid" style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:20,alignItems:'start'}}>
            ...
            <div>
              <div className="fu3" style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}>
                ...
                {photos.length>1&&<button className={`tab-btn ${activeTab==='gif'?'active':''}`} onClick={()=>setActiveTab('gif')}>
                  <span style={{fontSize:11,fontWeight:700,background:'linear-gradient(135deg,#f472b6,#818cf8)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>GIF</span>&nbsp;Animasi
                </button>}
              </div>
                  ...
                  <div style={{display:'flex',justifyContent:'center'}}>
                    {session.result_url ? (
                      <div className="photo-card" onClick={()=>setLightbox(session.result_url!)}
                        style={{borderRadius:14,overflow:'hidden',boxShadow:'0 8px 32px rgba(0,0,0,.5)',border:'1px solid rgba(255,255,255,.07)',maxWidth:previewW,width:'100%'}}>
                        <img src={session.result_url} alt="Photo strip" style={{width:'100%',display:'block'}}/>
                      </div>
                    ) : photos.length>0 ? (
                      <div className="photo-card" style={{borderRadius:14,overflow:'hidden',boxShadow:'0 8px 32px rgba(0,0,0,.5)',border:'1px solid rgba(255,255,255,.07)'}}>
                      ...
                      <div className="glass" style={{borderRadius:14,padding:'60px 40px',textAlign:'center',width:'100%'}}>
                        <div style={{width:44,height:44,borderRadius:'50%',border:'3px solid rgba(99,102,241,.4)',borderTopColor:'#6366f1',animation:'spin 1s linear infinite',margin:'0 auto 14px'}}/>
                        <p style={{color:'rgba(255,255,255,.3)',fontSize:14}}>Memproses foto...</p>
                      </div>
                    )}
                  </div>
                )}
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:10}}>
                    {photos.map((p,i)=>(
                      <div key={i} className="photo-card" onClick={()=>setLightbox(p.photo_url)}
                        style={{borderRadius:10,overflow:'hidden',boxShadow:'0 4px 16px rgba(0,0,0,.4)',border:'1px solid rgba(255,255,255,.06)',position:'relative'}}>
                        <img src={p.photo_url} alt={`Foto ${i+1}`} style={{width:'100%',aspectRatio:'1',objectFit:'cover',objectPosition:'center top',display:'block'}}/>
                        <div style={{position:'absolute',top:7,left:7,background:'rgba(0,0,0,.65)',borderRadius:5,padding:'2px 7px',color:'rgba(255,255,255,.8)',fontSize:11,fontWeight:700}}>{i+1}</div>
                        <div style={{position:'absolute',bottom:0,left:0,right:0,padding:'18px 8px 8px',background:'linear-gradient(to top,rgba(0,0,0,.75),transparent)',display:'flex',justifyContent:'center'}}>
                          <button onClick={e=>{e.stopPropagation();handleDownload(p.photo_url,`foto_${i+1}_${uuid.slice(0,8)}.jpg`)}}
                            style={{background:'rgba(255,255,255,.15)',border:'1px solid rgba(255,255,255,.2)',borderRadius:7,padding:'4px 10px',color:'white',fontSize:11,cursor:'pointer',display:'flex',alignItems:'center',gap:4,fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
                ...
                {activeTab==='gif'&&photos.length>0&&(
                  <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:20}}>
                    ...
                    <div style={{position:'relative'}}>
                      <div style={{position:'absolute',inset:-10,borderRadius:18,background:'linear-gradient(135deg,rgba(99,102,241,.25),rgba(236,72,153,.25))',filter:'blur(16px)',animation:'gif-glow .8s ease infinite'}}/>
                      <div style={{position:'relative',borderRadius:14,overflow:'hidden',border:'1px solid rgba(255,255,255,.1)',width:260}}>
                        ...
                        <div style={{position:'absolute',top:8,right:8,background:'rgba(0,0,0,.75)',borderRadius:5,padding:'2px 8px',color:'#f472b6',fontSize:10,fontWeight:700}}>
                    ...
                    {gifLoading&&(
                      <div style={{width:'100%',maxWidth:300}}>
                        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                          <Loader2 size={14} color="#a5b4fc" style={{animation:'spin .8s linear infinite'}}/>
                          <span style={{color:'rgba(255,255,255,.5)',fontSize:12}}>Membuat GIF... {gifProgress}%</span>
                        </div>
                        <div style={{height:4,background:'rgba(255,255,255,.08)',borderRadius:2,overflow:'hidden'}}>
                          <div style={{height:'100%',width:`${gifProgress}%`,background:'linear-gradient(90deg,#6366f1,#ec4899)',borderRadius:2,transition:'width .3s'}}/>
                        </div>
                      </div>
                    )}
                    ...
                    {gifError&&!gifLoading&&(
                      <div style={{width:'100%',maxWidth:300,textAlign:'center'}}>
                        <p style={{color:'#f87171',fontSize:13,marginBottom:10}}>{gifError}</p>
                        <button className="dl-btn" onClick={retryGif}
                          style={{background:'rgba(244,114,182,.1)',color:'#f472b6',border:'1px solid rgba(244,114,182,.2)',maxWidth:200}}>
                    ...
                    {gifUrl&&!gifLoading&&(
                      <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:12}}>
                        <img src={gifUrl} alt="GIF" style={{borderRadius:12,maxWidth:260,border:'1px solid rgba(255,255,255,.1)'}}/>
                        <button className="dl-btn" onClick={()=>handleDownload(gifUrl,`photobooth_gif_${uuid.slice(0,8)}.gif`)}
                          style={{background:'linear-gradient(135deg,#ec4899,#8b5cf6)',color:'white',maxWidth:260,boxShadow:'0 4px 20px rgba(236,72,153,.3)'}}>
            ...
            <div className="right-panel" style={{display:'flex',flexDirection:'column',gap:14}}>
              {session.result_url&&(
                <div className="glass fu3" style={{borderRadius:18,padding:20}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                    ...
                    <p style={{color:'white',fontSize:13,fontWeight:700}}>Photo Strip</p>
                  </div>
                  <p style={{color:'rgba(255,255,255,.25)',fontSize:11,marginBottom:14,paddingLeft:15}}>Hasil final dengan frame</p>
                  <button className="dl-btn" disabled={downloading==='strip'}
                    ...
                    {downloading==='strip'
                      ?<><div style={{width:14,height:14,border:'2px solid rgba(255,255,255,.3)',borderTopColor:'white',borderRadius:'50%',animation:'spin .8s linear infinite'}}/>Mengunduh...</>
              ...
              {photos.length>0&&(
                <div className="glass fu3" style={{borderRadius:18,padding:20}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                    ...
                    <p style={{color:'white',fontSize:13,fontWeight:700}}>Foto Individual</p>
                  </div>
                  <p style={{color:'rgba(255,255,255,.25)',fontSize:11,marginBottom:14,paddingLeft:15}}>{photos.length} foto tanpa frame</p>
                  <div style={{display:'flex',flexDirection:'column',gap:7}}>
                    {photos.map((p,i)=>(
                      <button key={i} className="dl-btn" disabled={downloading===`photo_${i}`}
                        ...
                        {downloading===`photo_${i}`?<div style={{width:13,height:13,border:'2px solid rgba(52,211,153,.3)',borderTopColor:'#34d399',borderRadius:'50%',animation:'spin .8s linear infinite'}}/>:<Download size={13}/>}
                        Foto {i+1}
                      </button>
                    ))}
                  </div>
              ...
              {photos.length>1&&(
                <div className="glass fu4" style={{borderRadius:18,padding:20}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                    ...
                    <p style={{color:'white',fontSize:13,fontWeight:700}}>GIF Animasi</p>
                  </div>
                  <p style={{color:'rgba(255,255,255,.25)',fontSize:11,marginBottom:14,paddingLeft:15}}>Dibuat otomatis di browser</p>
                  {gifUrl?(
                    <button className="dl-btn" onClick={()=>handleDownload(gifUrl,`photobooth_gif_${uuid.slice(0,8)}.gif`)}
                      style={{background:'linear-gradient(135deg,#ec4899,#8b5cf6)',color:'white',boxShadow:'0 4px 16px rgba(236,72,153,.25)'}}>
                      <Download size={14}/>Download GIF
                    </button>
                  ):gifLoading?(
                    <button className="dl-btn" disabled style={{background:'rgba(244,114,182,.08)',color:'#f472b6',border:'1px solid rgba(244,114,182,.15)'}}>
                      <div style={{width:13,height:13,border:'2px solid rgba(244,114,182,.3)',borderTopColor:'#f472b6',borderRadius:'50%',animation:'spin .8s linear infinite'}}/>
                      Membuat GIF... {gifProgress}%
                    </button>
                  ):gifError?(
                    <button className="dl-btn" onClick={retryGif}
                      style={{background:'rgba(244,114,182,.08)',color:'#f472b6',border:'1px solid rgba(244,114,182,.15)'}}>
              ...
              <div className="fu4" style={{textAlign:'center',paddingTop:4}}>
                <p style={{color:'rgba(255,255,255,.1)',fontSize:10,letterSpacing:'1.5px',fontFamily:'Poppins,sans-serif',textTransform:'uppercase'}}>Powered by</p>
                <p style={{color:'rgba(255,255,255,.25)',fontSize:15,fontWeight:700,fontFamily:'Poppins,sans-serif',marginTop:2}}>{clientName}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {lightbox&&(
        <div className="lb" onClick={()=>setLightbox(null)}
          ...
          <button onClick={()=>setLightbox(null)}
            style={{position:'fixed',top:16,right:16,background:'rgba(255,255,255,.1)',border:'1px solid rgba(255,255,255,.15)',borderRadius:'50%',width:40,height:40,color:'white',fontSize:18,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
            ✕
          </button>
        </div>
      )}
